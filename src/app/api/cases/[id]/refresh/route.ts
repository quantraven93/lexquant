...
export async function POST(
  ...
  const cleanCaseNumber =
    (caseData.case_number || "").replace(/^0+/, "") || "0";
  ...
  if (caseData.court_type === "HC" && caseData.petitioner) {
    ...
    try {
      ...
      const pageRes = await fetch(
        `${HC_BASE}/cases/ki_petres.php?state_cd=${stateCode}&dist_cd=1&court_code=1`,
        {
          headers: {
      ...
      const capRes = await fetch(
        `https://hcservices.ecourts.gov.in${capPath}`,
        {
          headers: {
            Cookie: pageCookies,
            "User-Agent": "Mozilla/5.0",
            Referer: `${HC_BASE}/cases/ki_petres.php`,
          },
      ...
      const allCookies = [
        pageCookies,
        ...(capRes.headers.getSetCookie?.() || []).map((c) => c.split(";")[0]),
      ]
        .filter(Boolean)
        .join("; ");
      const capAnswer = await solveCaptchaWithVision(
        Buffer.from(await capRes.arrayBuffer()),
        "text",
      );
      ...
      const searchName = (caseData.petitioner || "")
        .split(" ")
        .slice(0, 2)
        .join(" "); // First 2 words
      const searchBody = new URLSearchParams({
        ...
        court_code: "1",
        f: "Both",
        petres_name: searchName,
        rgyear: caseData.case_year || "",
        captcha: capAnswer,
      });
      const searchRes = await fetch(`${HC_BASE}/cases/ki_petres_qry.php`, {
        ...
        headers: {
          ...
          Cookie: allCookies,
          Referer: `${HC_BASE}/cases/ki_petres.php?state_cd=${stateCode}&dist_cd=1&court_code=1`,
        },
        body: searchBody.toString(),
        signal: AbortSignal.timeout(30000),
      });
      const searchText = await searchRes.text();
      if (searchText.length < 50 || searchText.includes("error1"))
        throw new Error("HC search failed or CAPTCHA wrong");

      // Find matching record by CNR
      const clean = searchText.startsWith("\ufeff")
      ...
      if (!hcCaseId || !hcToken)
        throw new Error("Could not find case in HC search results");
      ...
      const detailRes = await fetch(
        `${HC_BASE}/cases/o_civil_case_history.php`,
        {
          method: "POST",
          headers: {
            "User-Agent": "Mozilla/5.0",
      ...
      const regDate =
        detailHtml.match(/Registration Date.*?:\s*(\d{2}-\d{2}-\d{4})/i)?.[1] ||
        "";
      const nextHearing =
      ...
      if (filingDate) {
        const d = toISO(filingDate);
        if (d) updateData.filing_date = d;
      }
      if (regDate) {
        const d = toISO(regDate);
        if (d) updateData.registration_date = d;
      }
      if (nextHearing) {
        const d = parseHCDate(nextHearing);
        if (d) updateData.next_hearing_date = d;
      }
      ...
      return NextResponse.json(
        {
          error:
            "HC detail fetch failed: " +
            (hcErr instanceof Error ? hcErr.message : "Unknown error") +
            ". Try again.",
        },
        { status: 502 },
      );
  ...
  if (caseData.court_type === "SC" && caseStatus?.rawData?.sourceHtml) {
    const diaryMatch = (caseStatus.rawData.sourceHtml as string).match(
      /data-diary-no="(\d+)"\s*data-diary-year="(\d{4})"/,
    );
    if (diaryMatch) {
      console.log(
        `[Refresh] Fetching SC detail for diary ${diaryMatch[1]}/${diaryMatch[2]}`,
      );
      try {
        ...
        if (detailRes.ok) {
          ...
          const detailHtml =
            typeof detailData.data === "string" ? detailData.data : "";
          if (detailHtml.length > 100) {
            ...
            const judgesMatch = detailHtml.match(
              /Present\/Last Listed On[\s\S]*?<td[^>]*>[\s\S]*?\[([\s\S]*?JUSTICE[\s\S]*?)\]/i,
            );
            if (judgesMatch)
              caseStatus.judges = stripTags(judgesMatch[1])
                .replace(/and/gi, ", ")
                .trim();

            const filingMatch = detailHtml.match(
              /Filed on\s+(\d{2}-\d{2}-\d{4})/i,
            );
            if (filingMatch) caseStatus.filingDate = filingMatch[1];

            const disposalMatch = detailHtml.match(
            ...
            if (petAdv)
              caseStatus.petitionerAdvocate = petAdv.split(/\n|<br/)[0]?.trim();

            const respAdv = extractDetail("Respondent Advocate(s)");
            if (respAdv)
              caseStatus.respondentAdvocate = respAdv
                .split(/\n|<br/)[0]
                ?.trim();

            const cnr = extractDetail("CNR Number");
            if (cnr)
              caseStatus.rawData = { ...caseStatus.rawData, cnrNumber: cnr };

            console.log("[Refresh] Detail merged:", {
              judges: caseStatus.judges,
              filing: caseStatus.filingDate,
                ...
                return rows
                  .map((r) => {
                    ...
                    return dateMatch
                      ? {
                          date: dateMatch[1],
                          orderType: typeMatch ? typeMatch[1] : "Order",
  ...
  if (!caseStatus) {
    return NextResponse.json(
      {
        error:
          "Could not fetch case details from court. CAPTCHA may have failed. Try again.",
      },
      { status: 502 },
    );
  }
  ...
  const newTitle =
    (caseStatus.caseTitle && caseStatus.caseTitle !== "Unknown"
  ...
  if (caseStatus.currentStatus)
    updateData.current_status = caseStatus.currentStatus;
  ...
  if (caseStatus.petitionerAdvocate)
    updateData.petitioner_advocate = caseStatus.petitionerAdvocate;
  if (caseStatus.respondentAdvocate)
    updateData.respondent_advocate = caseStatus.respondentAdvocate;
  if (caseStatus.judges) updateData.judges = caseStatus.judges;
  if (caseStatus.nextHearingDate) {
    const d = toISO(caseStatus.nextHearingDate);
    if (d) updateData.next_hearing_date = d;
  }
  if (caseStatus.lastOrderDate) {
    const d = toISO(caseStatus.lastOrderDate);
    if (d) updateData.last_order_date = d;
  }
  if (caseStatus.lastOrderSummary)
    updateData.last_order_summary = caseStatus.lastOrderSummary;
  if (caseStatus.filingDate) {
    const d = toISO(caseStatus.filingDate);
    if (d) updateData.filing_date = d;
  }
  if (caseStatus.registrationDate) {
    const d = toISO(caseStatus.registrationDate);
    if (d) updateData.registration_date = d;
  }
  if (caseStatus.decisionDate) {
    const d = toISO(caseStatus.decisionDate);
    if (d) updateData.decision_date = d;
  }
